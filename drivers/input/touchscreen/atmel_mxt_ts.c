...
#ifdef CONFIG_TOUCHSCREEN_ATMEL_ENCRYPTION
...
#define CFG_ENCRYPTED		BIT(0)	/* 0x01 */
#define DEV_ENCRYPTED		BIT(1)	/* 0x02 */
#define MSG_ENCRYPTED   	BIT(2)	/* 0x04 */

/* Define macros */
#define CHECK_BIT(var, pos) ((unsigned int)(var) & pos)
#define SET_BIT(var, pos) (var |= pos)
#define CLEAR_BIT(var, pos) (var &= (~(pos)))
...
/* T68 datatype definitions */
#define T68_PDS_DTYPE		0x0005
#define T68_ENC_DTYPE		0x000C
#define T68_PWM_DTYPE		0x000A
#define T68_EDGE_SHAPE_DTYPE	0x000B
...
#define MXT_CIPHER_TIMEOUT	100 	/* msec */
...
#ifdef CONFIG_TOUCHSCREEN_ATMEL_ENCRYPTION
//******************************************************************************
/// \brief AES-128 key and IV
/// \Default keys for testing only
/// \Replace with customer key and IV before compile

/* Example keys for testing with below devices */
/* mXT640UD-002, mXT336UD-002, mXT2952TD_EN-2.0 */

static unsigned char ck_key[16] = {

	0xF4, 0x7A, 0x68, 0xFA, 0xDD, 0xB5, 0xCF, 0xA1,
	0x0F, 0xFF, 0xF8, 0x5A, 0xBE, 0x3E, 0xAD, 0x53
};

static unsigned char iv[16] = {

	0xEC, 0x90, 0xE4, 0x80, 0xDC, 0x1D, 0x6F, 0xE0,
	0x2D, 0xF9, 0x26, 0xCC, 0x25, 0x8C, 0x0F, 0xB2
};
...
struct mxt_data {
	...
	struct mutex rd_msg_lock;	/* Replace with i2c_lock? */
	u8 max_reportid;
	u32 config_crc;
	u32 info_crc;
	u8 bootloader_addr;
	...
	/* Msg enc state change */
	bool msg_enc_triggered;
...
static bool mxt_lookup_chips(struct mxt_data *data)
{
	...
	/* Add new devices supporting encryption */
	switch (family_id) {
		case 0xA6:
		case 0xA4:
			...
			if (variant_id & 0x80) {
				...
				dev_info(dev, "Device is using encryption\n");
				/* Set global block size for read */
				/* TBD - investigate removal of blocksize */
				data->enc_blocksize = 0x30;
			} else {
				CLEAR_BIT(data->encryption_state, DEV_ENCRYPTED);				
			}

			/* Identify chips here, add based on project */
			if (family_id == 0xA6) {
				switch (variant_id & 0x80) {
					case 0x14: /* 336UD-HA */
						dev_info(dev, "Found mXT366UD-HA\n");
						break;
					case 0x15: /* 640UD-HA */
						dev_info(dev, "Found mXT640UD-HA\n");
						break;
					case 0x16: /* 448UD-HA */
						dev_info(dev, "Found mXT448UD-HA\n");
						break;
					case 0x1C: /* 336UD-002 */
						dev_info(dev, "Found mXT336UD-002\n");
						break;
					case 0x17:  /* 640UD-002 */
					case 0x1D:  /* 228UD-002 */
			...
			} else if (family_id == 0xA4) {
				switch (variant_id & 0x80) {
					case 0x59: /* mXT2952TD-EN */
						dev_info(dev, "Found mXT2952TD-EN\n");
						break;
...
#ifdef CONFIG_TOUCHSCREEN_DIAGNOSTICS_T33
/* T33 CRC message based on lookup table of 256 elements
*  Initialize look up table first
*/
			  ...
			  const void *val, struct mxt_data *data, u8 flag)
{
	...
	/*  Flag for sysfs write from mxt-app, device encrypted */
	if ((flag & F_W_SYSFS_ID) == F_W_SYSFS_ID) {
		/* Get real length minus embedded datasize bytes*/
		message_length = len - 2; 
		bytesToWrite = len - 2;
	} else { /* Normal writes, internal or sysfs, non-encyrpted */
		/* Make copy of original length */
		message_length = len;		
		bytesToWrite = len;
	}

	/* wbuf handles messages with embedded datasize */
	/* tbuf handles messages with no enc datasize embedded */
	...
	if (CHECK_BIT(data->encryption_state, CFG_ENCRYPTED)) {
		...
		/* If sysfs write, separate data size bytes and data to be written */
		if ((flag & F_W_SYSFS_ID) == F_W_SYSFS_ID) {
			/* Initially only copy incoming datasize into wbuf */
			/* Datasize is updated later */
			memcpy(&wbuf[0], (u8*)val, 2); /* Copy just embedded datasize */
			/* save copy of datasize */
			data->enc_datasize = (wbuf[0] | (wbuf[1] << 8));
			/* tbuf contains data to be written only */
			memcpy(&tbuf[0], ((u8*)val + 2), len-2);
		}		
	} else if (CHECK_BIT(data->encryption_state, MSG_ENCRYPTED)) {
		memcpy(&tbuf[0], ((u8*)val + 2), len-2);
	...
	do {
		...
		if (CHECK_BIT(data->encryption_state, DEV_ENCRYPTED)) {
			...
			/* TBD -- Do I need to do this, as T5 is less than T38 addr */
			/* TBD -- Combine this code with above if statement */
			if ((CHECK_BIT(data->encryption_state, MSG_ENCRYPTED)) &&
				(!CHECK_BIT(data->encryption_state, CFG_ENCRYPTED))) {
				/* Data size is 0x00 if writing to config with MSG enabled */
				wbuf[0] = 0x00;
				wbuf[1] = 0x00;
			} else {
				/* load the datasize byte into buffer */
			...
			if ((flag & F_W_SYSFS_ID) == F_W_SYSFS_ID) {
				...
				/* move msg_count here, plus 2 for data size embedded */
				/* Plus 2 for write addr */
				msg_count = message_length + 2;	/* TBD -- Moved from above */
			} else {
...
#ifdef CONFIG_TOUCHSCREEN_ATMEL_ENCRYPTION
static void mxt_skcipher_cb (struct crypto_async_request *req, int error)
{
	
	struct tcrypt_result *result = req->data;

	if (error == -EINPROGRESS) {
		printk(KERN_INFO "Error: cipher encdec in process\n");
		return;
	}

	result->err = error;
	/* flag that cipher is done */
	complete(&result->completion);

}

static unsigned int mxt_skcipher_encdec(struct mxt_data *data, struct skcipher_def *sk, int enc)
{
	...
	case -EINPROGRESS:
	case -EBUSY:
		ret = mxt_wait_for_completion(data, &sk->result.completion,
			MXT_CIPHER_TIMEOUT);

		if (ret < 0) {
			dev_warn(&client->dev, "Cipher wait interrupted\n");
		} else if (ret == -ETIMEDOUT) {
			dev_err (&client->dev, "Cipher timed out\n");
		} else {
			dev_dbg(&client->dev, "Jiffies remaining %d", ret);
	...
	default:
		dev_dbg(&client->dev, "skcipher encrypt returned with %d result %d\n",
		 ret, sk->result.err);
		break;
	}

	/* init for next skcipher enc_dec */
	init_completion(&sk->result.completion);

	return ret;
}

static int mxt_skcipher_req(struct mxt_data *data, u8 *databuf, 
	int data_length, bool enc_dec) {
	...
	if (IS_ERR(skcipher)) {
		...
		dev_err(&client->dev, "ERROR: %d could not allocate cipher.  %s, line %d\n",
	...
	if(!req) {
		dev_err(&client->dev, "Could not allocate skcipher request\n");
		return -ENOMEM;
	} 
	...
	if (!ivdata) {
		dev_err(&client->dev, "Could not allocate memory for iv\n");
		return -ENOMEM;
	}

	keydata = kmalloc(16, GFP_KERNEL);
	if (!keydata) {
		dev_err(&client->dev, "Could not allocate memory for key\n");
		return -ENOMEM;
	}

	/* Copy current set of keys into buffer */
	/* TBD - could bypass use of buffer */
	memcpy(keydata, data->cust_key, 16);

	error = crypto_skcipher_setkey(skcipher, keydata, 16);
	if(error) {
		dev_err(&client->dev, "Could not set keys\n");
	} 
	/* Assume default keys present from driver probe */
	/* TBD - could bypass use of buffer */
	memcpy(ivdata, data->init_vector, 16);

	sk.tfm = skcipher;
	sk.req = req;
	
	/* insert data into scatter gather list */
	sg_init_one(&sk.sg, databuf, data_length);

	skcipher_request_set_callback(req, CRYPTO_TFM_REQ_MAY_BACKLOG,
		mxt_skcipher_cb, &sk.result);

	skcipher_request_set_crypt (req, &sk.sg, &sk.sg, data_length, ivdata);

	init_completion(&sk.result.completion);

	error = mxt_skcipher_encdec(data, &sk, enc_dec);
	if (error) {
		dev_err(&client->dev, 
			...
			u16 flags)
{
	...
	u16 bytesReadOffset = 0;
	u16 msg_length = 0;
	size_t w_hdr_size = 0;

	/* Make copy of the original incoming length */
	/* Expect to read all bytes and deliver back full length */
	bytesToRead = len;	//Total bytes to read
	msg_length = len; //Variable message length, will change
	...
	/* Check if encrypted and modify message length, if needed */
	/* msg_length, <48, padded or left as is */
	...
	if (CHECK_BIT(data->encryption_state, MSG_ENCRYPTED) &&
		...
		if (reg >= (data->T38_address + data->T38_obj_size)) {
			if (len % 16 != 0) {
				bytesToRead = (16 - (len % 16) + len);
			}
			
		/* modify msg_length based on incoming size */
		/* Already padded at this time */
			if (bytesToRead > data->enc_blocksize) {
	...
	/* Holds only the total msg buffer size to be read at a time */
	readbuf = kmalloc(msg_length, GFP_KERNEL);
	if (!readbuf)
		return -ENOMEM;

	do { /* TBD - Need update to read encrypted blocks */

		ret = i2c_master_recv(data->client, readbuf, msg_length);

		if (ret == msg_length) {
			memcpy(&databuf[bytesReadOffset], &readbuf[0], msg_length);
			ret = 0;
		} else {
			ret = -EIO;
		...
		/* Update read offset */
		bytesReadOffset += msg_length;
		/* Reduce by padded or unpadded total bytes to read */
	...
	if (CHECK_BIT(data->encryption_state, DEV_ENCRYPTED)) {
		...
		if (((CHECK_BIT(data->encryption_state, CFG_ENCRYPTED)) &&
			(reg >=(data->T38_address + data->T38_obj_size))) || 
			((CHECK_BIT(data->encryption_state, MSG_ENCRYPTED)) &&
			(reg == data->T5_address))) {
			ret = mxt_skcipher_req(data, databuf, totalBytesToRead, 0);
			if (ret < 0) {
				dev_err(dev, "Error in decode. Data may be invalid.\n");
			}
		}
	}
#endif
...
static int mxt_check_encryption(struct mxt_data *data)
{
	struct device *dev = &data->client->dev;
	struct i2c_client *client = data->client;
	int ret = 0;
	...
	if (data->T2_address) {
		...
		if (data->is_key_loaded == false) {
			memcpy(data->cust_key, ck_key, 16);
			memcpy(data->init_vector, iv, 16);
			data->is_key_loaded = true;
		}
#endif
		if (!(data->crc_enabled)) {
			  ...
			  u8 value, bool wait)
{
	...
	if (!(data->crc_enabled)){
		ret = mxt_write_reg(data->client, reg, value, data);
	} else {
...
static int mxt_disable_p2p_sct_on_error(struct mxt_data *data)
{
	...
	if (!data->crc_enabled)	{
		...
		ret = mxt_write_reg(client, T8_selfcap_ofs, val, data);
	}
	...
	if (!data->crc_enabled)	{
		...
		ret = mxt_write_reg(client, T100_p2p_ofs, val, data);
	}
	/* Backup the content */
...
static int mxt_read_and_process_messages(struct mxt_data *data, u8 count, bool crc8)
{
	...
	/* Safety check for T5_msg_buf */
	if (count > data->max_reportid) {
		dev_info(dev, "Error: count > max_reportID\n");
		return -EINVAL;
	}
	...
	for (i = 0; i < count; i++) {
		if (data->crc_enabled){
			ret = __mxt_read_reg_crc(data->client, data->T5_address,
				data->T5_msg_size, data->T5_msg_buf + 1, data, crc8);
		} else {
			/* Process remaining messages if necessary */
			ret = mxt_read_block(data, data->T5_address, 
				data->T5_msg_size, data->T5_msg_buf + 1, 0);
		}
		...
		if (data->T5_msg_buf[1] == 0xFF) {
			dev_dbg(dev, "Invalid message reached\n");
			break;
		}

		ret = mxt_proc_message(data, data->T5_msg_buf + 1);
...
static irqreturn_t mxt_process_messages_t44_t144(struct mxt_data *data)
{
	...
	/* For new HA parts, read T144 count and CRC */
	/* Just read byte count if encrypted */

	if (CHECK_BIT(data->encryption_state, MSG_ENCRYPTED)) {
		ret = mxt_read_block(data, data->T44_address,
			1, data->T5_msg_buf, false);
	} else if (!(data->crc_enabled)) {
	...
	count = data->T5_msg_buf[0];

	if ((data->crc_enabled) && count > 0){
		/* Read first T5 message */
		ret = __mxt_read_reg_crc(data->client, data->T5_address,
			data->T5_msg_size, data->T5_msg_buf + 1, data, true);
	} else if ((CHECK_BIT(data->encryption_state, MSG_ENCRYPTED)) &&
		count > 0) {
		ret = mxt_read_block(data, data->T5_address,
			16, data->T5_msg_buf + 1, false);
	}
	...
	/* Only send message not the count */
	/* Only for normal reads */
	/* If found 0xFF, just leave */
	ret = mxt_proc_message(data, data->T5_msg_buf + 1);
	if (ret < 0) {
...
static int mxt_process_messages_until_invalid(struct mxt_data *data)
{
	...
	/* Expect to read up to all messages */
	count = data->max_reportid;

	/* Read all messages until we force an invalid */
	do {
...
static u32 mxt_calculate_crc(u8 *base, off_t start_off, off_t end_off, u32 crc)
{
...
/// \return #mxt_rc
static int mxt_t68_check_power_cfg(struct mxt_data *data)
{
	struct device *dev = &data->client->dev;
	uint8_t buf[2];
	int ret = 0;
...
static int mxt_t68_write_length(struct mxt_data *data, u16 length)
{
	struct device *dev = &data->client->dev;
	struct i2c_client *client = data->client;
	int ret = 0;
...
static int mxt_upload_t68_payload(struct mxt_data *data, struct mxt_cfg *cfg)
{
	...
	data->t68_datasize = data->T68_obj_size - 9;
  	...
  	dev_info(dev, "T68 payload programming done");
	...
	while (cfg->raw_pos < cfg->raw_size) {
				...
				(instance & 0x8000)) 
		{
			...
			data->t68_data_crc = ((data->t68_buf[size-4] << 24) |
				(data->t68_buf[size-3] << 16) | (data->t68_buf[size-2] << 8) |
				(data->t68_buf[size-1]));
			...
			if (data->t68_datatype == T68_ENC_DTYPE) {
				...
				if (data->t68_buf[32] & 0x04) {
        			dev_info(dev, "Enabling T5 msg encryption");
          			data->msg_enc_triggered = true;
      			} else if (data->t68_buf[32] & 0x02) {
        			dev_info(dev, "Enabling config encryption");
        			data->msg_enc_triggered = false;
      			} else if ((data->t68_buf[32] & 0x06) == 0x06) {
        			dev_info(dev, "Enabling T5 msg and cfg encryption");
        			data->msg_enc_triggered = true;
      			} else if ((data->t68_buf[32] & 0x06) == 0x00) {
      				if (CHECK_BIT(data->encryption_state, MSG_ENCRYPTED)) {
        				data->msg_enc_triggered = true;
        			}
        			dev_info(dev, "Disabling device encryption");
      			}
			}
		...
		/* Skip is cfg is encrypted, messages do not pertain */
		if (!(CHECK_BIT(data->encryption_state, CFG_ENCRYPTED))) {
...
static void mxt_print_info_block(struct mxt_data *data);
...
static int mxt_update_cfg(struct mxt_data *data, const struct firmware *fw)
{
	...
	} else if (!strncmp(cfg.raw, MXT_CFG_MAGIC_V4, strlen(MXT_CFG_MAGIC_V4))) {
	...
	if ((cfg_version == 0x03) || (cfg_version == 0x04)) {
		...
		if (sscanf(cfg.raw + cfg.raw_pos, "%s%d%n", tmp, &num_of_devices, &offset) != 2) {
			dev_err(dev, "Failed to get number of devices");
		}

		if (!(strcmp("DEVICE_NO ", tmp))) {
			dev_err(dev, "DEVICE_NO header not found");
			goto release_raw;
		}
	...
	if ((cfg.info.variant_id & 0x7f) != (data->info->variant_id & 0x7f)) {
	...
	} else if ((cfg.info.variant_id & 0x80) || (cfg_version > 0x02)) {	/* Check if upper MSbit set */
		dev_warn(dev, "Warning: Infoblock CRC does not match, device encrypted\n");
	} else {
		dev_warn(dev, "Warning: Infoblock CRC does not match: "
	...
	if (cfg_version == 0x04) {
		if (sscanf(cfg.raw + cfg.raw_pos, "%s%n", tmp, &offset) != 1) {
			dev_err(dev, "Failed to find beginning bracket");
		}

		/* Don't do anything with header, just skip and update position in raw array */
		cfg.raw_pos += offset;
	}
	...
	if (ret) {
		dev_err(dev, "Read Info Block failed\n");
	} else {
		dev_info(dev, "Read Infoblock successful\n");
	}
	
	ret = mxt_check_encryption(data);
	if (ret) {
		dev_dbg(dev, "Check encryption state failed\n");
	} else {
		dev_info(dev, "Check encryption state successful\n");
	}

	if (data->msg_enc_triggered) {
		enable_irq(data->irq);
		dev_info(dev, "Interrupt enabled\n");
	}

	mxt_print_info_block(data);
...
static void mxt_free_object_table(struct mxt_data *data)
{
	...
	kfree(data->T5_msg_buf);
	data->T5_msg_buf = NULL;
	data->T5_address = 0;
	data->T5_msg_size = 0;
	data->T6_reportid = 0;
	data->T7_address = 0;
				  ...
				  struct mxt_object *object_table)
{
	...
	for (i = 0; i < data->info->object_num; i++) {
		...
		/* For each obj, size is increased by padding */
		/* then multiplied by each instance */
		if (((u16)(mxt_obj_size(object) % 16) == 0x0000) && 
			((u16)(mxt_obj_size(object) != 0x0000))) {

			obj_size = (u16)(mxt_obj_size(object));
		} else {
			obj_size = ((16 - (u16)(mxt_obj_size(object) % 16)) +
				mxt_obj_size(object));
		}
		
		/* Total physical size of encrypted cfg */
		/* With embedded datasize bytes and instance */
		obj_size = (obj_size * num_instances) + 2;
		...
		switch (object->type) {
		case MXT_GEN_ENCRYPTIONSTATUS_T2:
			data->T2_address = object->start_address;
			break;
		case MXT_GEN_MESSAGE_T5:
		...
		case MXT_SPT_USERDATA_T38:
			...
			data->T38_obj_size = mxt_obj_size(object);
			break;
		case MXT_SPT_DYNAMICCONFIGURATIONCONTAINER_T71:
		...
		/* Keep track for possible encrypted object */
	...
	if (CHECK_BIT(data->encryption_state, MSG_ENCRYPTED)) {
		/* Increase buff to be 16 + 2 data size bytes */
		/* one message at a time, TBD - block of 48 bytes later */
		data->T5_msg_buf = kcalloc(data->max_reportid, 
			18, GFP_KERNEL);
	} else {
		data->T5_msg_buf = kcalloc(data->max_reportid,
				data->T5_msg_size + 1, GFP_KERNEL);
	}
...
static void mxt_print_info_block(struct mxt_data *data)
{
	...
	dev_info(&client->dev, "Matrix size: X%u Y%u\n",
		data->info->matrix_xsize, data->info->matrix_ysize);

	dev_info(&client->dev, "Information Block CRC: 0x%06X\n\n",
		data->info_crc);
}
...
static int mxt_read_info_block(struct mxt_data *data)
{
	...
	/* May want to removed, duplicate if using mxt-app- else okay */
	dev_dbg(&client->dev,
		 "Family: %u Variant: %u Firmware V%u.%u.%02X Objects: %u\n",
		 data->info->family_id, data->info->variant_id,
		 data->info->version >> 4, data->info->version & 0xf,
	...
	error = mxt_check_encryption(data);
	if (error) {
...
static int mxt_touchscreen_parse_properties(struct mxt_data *data)
{
	struct device *dev = &data->client->dev;
	u32 val = 0;
	u8 aux = 0;
	
	if (!device_property_read_u32(dev, "atmel,x-range", &val)) {
		data->max_x = val;
	} else {
		dev_err(dev, "failed to parse '%s' property", "x-range");
		return -EINVAL;
	}

	if (!device_property_read_u32(dev, "atmel,y-range", &val)) {
		data->max_y = val;
	} else {
		dev_err(dev, "failed to parse '%s' property", "y-range");
		return -EINVAL;
	}

	if (!device_property_read_u32(dev, "atmel,xsize", &val)) {
		data->xsize = val;
	} else {
		dev_err(dev, "failed to parse '%s' property", "xsize");
	}

	if (!device_property_read_u32(dev, "atmel,ysize", &val)) {
		data->ysize = val;
	} else {
		dev_err(dev, "failed to parse '%s' property", "ysize");
	}

	if (!device_property_read_u32(dev, "atmel,tch-aux", &val)){
		data->t100_tchaux = val;

		aux = MXT_T100_AUX_OFFSET;

		if (data->t100_tchaux & MXT_T100_TCHAUX_VECT)
			data->t100_aux_vect = aux++;

		if (data->t100_tchaux & MXT_T100_TCHAUX_AMPL)
			data->t100_aux_ampl = aux++;

		if (data->t100_tchaux & MXT_T100_TCHAUX_AREA)
			data->t100_aux_area = aux++;
			
	} else {
		dev_err(dev, "failed to parse '%s' property", "tch-aux");
		return -EINVAL;
	}

	data->xy_switch = device_property_read_bool(dev, "xyswitch");
	data->inverty = device_property_read_bool(dev, "inverty");
	data->invertx = device_property_read_bool(dev, "invertx");

	dev_dbg(dev, "T100 aux mappings vect:%u ampl:%u area:%u\n",
...
#ifdef CONFIG_TOUCHSCREEN_ATMEL_ENCRYPTION
static ssize_t mxt_enc_key_show(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	struct mxt_data *data = dev_get_drvdata(dev);
	int count = 0;
...
static ssize_t mxt_iv_data_show(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	struct mxt_data *data = dev_get_drvdata(dev);
	int count = 0;
...
static ssize_t mxt_enc_key_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct mxt_data *data = dev_get_drvdata(dev);
	off_t key_pos = 0;
	...
	for (i=0; i<16; i++) {
		ret = sscanf(buf + key_pos, "%02x,%n", (unsigned int *)(data->cust_key + i), 
			&offset);
...
static ssize_t mxt_iv_data_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct mxt_data *data = dev_get_drvdata(dev);
	int offset = 0;
	...
	for (i=0; i < 16; i++) {
		ret = sscanf(buf + key_pos, "%02x,%n", (unsigned int *)(data->init_vector + i), 
			&offset);
				       ...
				       size_t *count)
{
	...
	/* Needed if changing encryption state using mxtapp */
	/* Indicates to driver to check encryption steate */
	if (off == 0x00) {
	...
	size_t count)
{
	...
	/* TBD - Verify that CFG_ENCRYPTED is enough here */
	/* Only check for CFG encrypted, not needed if MSG encrypted */
	if (CHECK_BIT(data->encryption_state, DEV_ENCRYPTED)) {
		flag = F_W_SYSFS_ID;
	}
...
#ifdef CONFIG_TOUCHSCREEN_ATMEL_ENCRYPTION
static DEVICE_ATTR(enc_key, S_IWUSR | S_IRUGO, mxt_enc_key_show, mxt_enc_key_store);
static DEVICE_ATTR(iv_data, S_IWUSR | S_IRUGO, mxt_iv_data_show, mxt_iv_data_store);
#endif
...
#endif
#ifdef CONFIG_TOUCHSCREEN_ATMEL_ENCRYPTION
	&dev_attr_enc_key.attr,
	&dev_attr_iv_data.attr,
#endif
	NULL
};
...
static void mxt_start(struct mxt_data *data)
{
	...
	default:
		...
		mutex_lock(&data->rd_msg_lock);
		mxt_process_messages_until_invalid(data);
		mutex_unlock(&data->rd_msg_lock);
		//data->irq_processing = true;
